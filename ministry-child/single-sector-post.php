	...
	<div class="content-wrapper marT90">
		<div class="row">
			<div class="col-md-12">
				<div class="main-content sector-details hasSidebar">
					<div class="sdetails-left">
						<ul class="sub-menu">
							...
							<?php  if(have_rows('sector_sub_title_content')){
								$k=1; while(have_rows('sector_sub_title_content')):the_row();?>
							<li <?php if($stsID == $k){ echo 'class="active-sdetails"';}?>><a href="<?php if($stsID == $k){ echo 'javascript:void(0)';} else {echo get_permalink().'?stsID='.$k;}?>"><?php the_sub_field('sub_sec_title');?></a></li>
							<?php $k++; endwhile; }?>
						</ul>
					</div>
					<div class="sdetails-right">
						...
						<?php } else {?>
							<div class="sector-info state-info">
								<?php if(have_rows('sector_sub_title_content')){ $m=1;
								while(have_rows('sector_sub_title_content')):the_row();
								if($stsID == $m){?>
								<h5 class="sec-title"><?php the_sub_field('sub_sec_title');?></h5>
								<div class="sub-content"><p><?php the_sub_field('sub_sec_content');?></p></div>
								<?php } $m++; endwhile; }?>
							</div>
						<?php }?>
					</div>
				...
				<?php if(have_rows('sector_galleries')){ ?>
				...
				<div class="photoLists-slider">
					...
					<?php while(have_rows('sector_galleries')):the_row();?>
					...
					<?php $j=0; while(have_rows('sector_galleries')):the_row();?>
				...
				<?php if(have_rows('sector_youtube_video_ids')){ ?>
				...
				<?php function get_youtube_title($ref) {
						...
						<h2 class="heading_trade_half">Videos</h2>
							...
							<?php while(have_rows('sector_youtube_video_ids')):the_row();
