...
<script type="text/javascript">
	jQuery(document).ready(function(){
		var curTabName = jQuery('.sector-details .sdetails-left ul li.active-sdetails').text();
		jQuery('.single-country-post .customBrc').append(jQuery('<i>»</i><span property="itemListElement" typeof="ListItem"><span property="name">'+curTabName+'</span><meta property="position" content="5"></span>'));
	});
</script>
	...
	<div class="content-wrapper marT90">
		<div class="row">
			<div class="col-md-12">
				<div class="main-content sector-details hasSidebar">
					<div class="sdetails-left">
						<ul class="sub-menu country-tabs">
							<li <?php if(!isset($stsID) && $curPID != 3454){ echo 'class="active-sdetails"';}?>><a href="<?php if(isset($stsID) || $curPID == 3454){ the_permalink();} else {echo 'javascript:void(0)';}?>">Country Profile</a></li>
							<?php  if(have_rows('country_tabs_title_content')){
								$k=1; while(have_rows('country_tabs_title_content')):the_row();?>
							<li <?php if($stsID == $k){ echo 'class="active-sdetails"';}?>><a href="<?php if($stsID == $k){ echo 'javascript:void(0)';} else {echo get_permalink().'?stsID='.$k;}?>"><?php the_sub_field('tab_title');?></a></li>
							<?php $k++; endwhile; }?>
							<?php /*?><li <?php if(!isset($stsID) && $curPID == 3454){ echo 'class="active-sdetails"';}?>><a href="<?php if(isset($stsID) || $curPID != 3454){ echo get_permalink(3454).'?acID='.$curPID;} else {echo 'javascript:void(0)';}?>">Agreements</a></li><?php */?>
						</ul>
					</div>
					<div class="sdetails-right">
						...
						<?php } else {?>
							<div class="sector-info state-info">
								<?php if(have_rows('country_tabs_title_content')){ $m=1;
								while(have_rows('country_tabs_title_content')):the_row();
								if($stsID == $m){?>
								<h5 class="sec-title"><?php the_sub_field('tab_title');?></h5>
								<div class="sub-content"><p><?php the_sub_field('tab_content');?></p></div>
								<?php } $m++; endwhile; }?>
							</div>
						<?php }?>
					</div>
				...
				<?php if(have_rows('country_gallery_images')){ ?>
				...
				<div class="photoLists-slider">
					...
					<?php while(have_rows('country_gallery_images')):the_row();?>
					...
					<?php $j=0; while(have_rows('country_gallery_images')):the_row();?>
				...
				<?php if(have_rows('country_youtube_video_ids')){ ?>
				...
				<?php function get_youtube_title($ref) {
						...
						<h2 class="heading_trade_half">Videos</h2>
							...
							<?php while(have_rows('country_youtube_video_ids')):the_row();
				...
				<h2 class="secTitle">News</h2>
					...
					<li>
						...
						<a href="<?php the_permalink();?>">
						    <h4>
							<?php 
                            $content_news = get_post($post->ID);
				            $content_news1 = $content_news->post_content;
                                 $shorttitle = wp_trim_words( $content_news1, $num_words = 5, $more = '...' );
								  echo $shorttitle;
                            ?>
							</h4>
						</a>
